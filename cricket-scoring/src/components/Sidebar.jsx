const menus = [
  { label: "Scoring", icon: "✏️", active: true },
  { label: "Scoreboard", icon: "📋" },
  { label: "Commentary", icon: "🎙️" },
  { label: "Players", icon: "👤" },
  { label: "Partnerships", icon: "🤝" },
  { label: "Wagon Wheel", icon: "🎡" },
  { label: "Manhattan", icon: "📊" },
  { label: "Reports", icon: "📄" },
  ...
  return (
    <div className="w-48 h-screen bg-[#0f172a] text-white flex flex-col shrink-0 border-r border-slate-800">
      ...
      <div className="px-4 py-3 flex items-center gap-1 border-b border-slate-800">
        <span className="text-white font-extrabold text-base tracking-tight">CRIC</span>
      ...
      <div className="mx-2 mb-3 bg-slate-800 rounded-lg p-3 text-xs border border-slate-700">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="bg-blue-600 text-white text-[10px] px-1.5 rounded font-bold">P1</span>
        </div>
        <div className="text-[10px] text-slate-300">
          Current Run Rate <span className="text-white font-semibold">{crr}</span>
        </div>
        <div className="text-[10px] text-slate-300 mt-0.5">
          Required Run Rate <span className="text-white font-semibold">{rrr}</span>
        </div>
      </div>
    </div>
  );
